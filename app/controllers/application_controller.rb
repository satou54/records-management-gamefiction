class ApplicationController < ActionController::Base
  ...
  def levelUpAndDown(task_id, action_day, action)
    ...
    # 1週間の実績を配列で取得
    week_of_actions = ActionRecord.weekOfActions(task_id, from, to)

    # 1週間の実績を合計
    week_of_action = week_of_actions.sum

    # 総経験値を取得
    total_experience_point = UserLevel.getTotalExperiencePoint(current_user.id)

    # レベルを取得
    level = UserLevel.getUserLevel(current_user.id)
    before_level = level

    # 現在のレベルの必要経験値を取得
    current_level_required_experience_point = Level.getRequreidExperiecePoint(level)

    # 次レベルの必要経験値を取得
    next_level_required_experience_point = Level.getRequreidExperiecePoint(level + 1)
    ...
    # 既にデータが存在しているかチェック
    if (ActionRecord.checkActionRecord?(action_day, task_id, current_user.id).nil?)
      # データが存在しない場合
      ...
      if (total_experience_point >= next_level_required_experience_point)
        ...
        # レベルアップ処理後のレベルを登録
        UserLevel.uploadUserLevel(current_user.id, level)
      end
    else
      # 一致するデータがある場合
      test = ActionRecord.find_by(action_day: action_day, task_id: task_id, user_id: current_user.id)

      # 今回修正された実績と登録されている実績との差分を出す
      difference = action.to_f - test.action.to_f
    ...
    # 総経験値を登録
    UserLevel.uploadUserTotalExperiencePoint(current_user.id, total_experience_point)

    # 次のレベルアップに必要な経験値を計算する
    until_levelup = Level.getNextLevelRequreidExperiencePoint(level, total_experience_point)
